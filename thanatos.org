...
#+begin_src emacs-lisp
(defun Tm-update-title ()
  (pcase-class-name
    ("fi" (exwm-workspace-rename-buffer (format "Firefox: %s" exwm-title))
     (sede-line-format nil))

    ("obn" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("Alty" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("kr(exwm-input-release-keyboard)
     (sede-line-format nil))

    ("Bl" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("Giexwm-input-release-keyboard)
     (sede-line-format nil))

    ("di" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("Bien" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("ffx11.exe" (exwm-input-release-keyboard)
     (sede-line-format nil))

    ("XIcher.Core" (exwm-input-release-keyboard)
     (sede-line-format nil))))
#+end_sr
